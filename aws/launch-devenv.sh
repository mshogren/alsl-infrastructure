...
aws ec2 describe-images --image-id ami-0bbe6b35405ecebdb --query 'Images[0].BlockDeviceMappings' > mapping.json

sed -i 's/8,/16,/' mapping.json
sed -i '/"Encrypted": false/d' mapping.json
sed -i 's/"gp2",/"gp2"/' mapping.json

aws ec2 run-instances --image-id ami-0bbe6b35405ecebdb --key-name $USER --security-groups alsl-ec2-dev-sg --instance-type t2.micro --block-device-mappings file://mapping.json --iam-instance-profile Name=alsl-ec2-dev --user-data file://userdata.sh
